...
describe("App", () => {
  let wrapper;
  const mockAlbums = [
    { trackName: "Hello", genre: "Rock" },
    { trackName: "Yellow", genre: "Folk" }
  ];
...
describe("mapStateToProps", () => {
  it("should return an array of albums", () => {
    const mockState = {
      albums: [
        { albumName: "Johanna", genre: "Rock" },
        { albumName: "Eoah", genre: "Country" }
      ],
      error: "error",
      user: 1
    };
...
describe("mapDispatchToProps", () => {
  ...
  it("calls storeFavorites with an array of favorites when storeFavorites is called", () => {
    const mockDispatch = jest.fn();
    const actionToDispatch = storeFavorites([
      { albumName: "Johanna", genre: "Rock" },
      { albumName: "Eoah", genre: "Country" }
    ]);
    const mappedProps = mapDispatchToProps(mockDispatch);
    mappedProps.storeFavorites([
      { albumName: "Johanna", genre: "Rock" },
      { albumName: "Eoah", genre: "Country" }
    ]);
    expect(mockDispatch).toHaveBeenCalledWith(actionToDispatch);
